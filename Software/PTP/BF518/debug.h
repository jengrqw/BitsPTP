...
/* if debug printing is enabled */
	...
	/* if printing to a file */
    #if defined(__DEBUG_FILE__)
        #undef _PRINT_CYCLES
        #define __DEBUG_FILE_NAME__ "ptp_debug.txt"	/* output file name */
        ...
        #define U_DEBUG_PRINT(fmt,...) fprintf(DEBUG_STREAM,fmt, ##__VA_ARGS__);
	...
	/* else if printing to the UART */
		...
		#define U_DEBUG_PRINT(fmt,...)		snprintf(UART_DEBUG_BUFFER, UART_DEBUG_BUFFER_LINE_SIZE, fmt, ##__VA_ARGS__); \
											UART_DEBUG_PRINT();
