...
//  if(ret==ptpClock->netPath.eventSock+1)
//  {
...
void handleSync(MsgHeader *header, Octet *msgIbuf, ssize_t length, TimeInternal *time, Boolean isFromSelf, RunTimeOpts *rtOpts, PtpClock *ptpClock)
{
  ...
  default:
      ...
      || ptpClock->clock_communication_technology == PTP_DEFAULT )
    {
      ...
      {
          DBGV( "Get Sync as master at %d:%d\n", time->seconds, time->nanoseconds );         /* JW for debug only */

          /* get the precision time stamp for the previously sent SYNC message */
          UInteger32 retNumberOfSeconds=0;
          UInteger32 retNumberOfNanoSeconds=0;
          UInteger16 overflowCount;
          TimeInternal sendTime;
          TimeInternal diffTime;
          while( 1 )
          {
              if( getTxStamp( &retNumberOfSeconds, &retNumberOfNanoSeconds, &overflowCount ) )
              {
                  sendTime.seconds = retNumberOfSeconds;
                  sendTime.nanoseconds = retNumberOfNanoSeconds;
              }
              else
              {
                  ERROR( "No TX time stamp capture for unicasting SYNC, set timestamp to 0\n" );
                  time->seconds = 0;
                  time->nanoseconds = 0;
                  break;
              }
              subTime( &diffTime, time, &sendTime );
  //            if( diffTime.seconds == 0 && diffTime.nanoseconds < 900000000 ) /* 900ms */
              {
                  *time = sendTime;
                  break; /* got the reasonable time stamp */
              }
          }

          DBGV( "Use sync sent time %d:%d\n", time->seconds, time->nanoseconds);
...
void handleDelayReq(MsgHeader *header, Octet *msgIbuf, ssize_t length, TimeInternal *time, Boolean isFromSelf, RunTimeOpts *rtOpts, PtpClock *ptpClock)
{
  ...
  case PTP_SLAVE:
    ...
    break;
      ...
      /* get the precision time stamp for the previously sent delay_req message */
      ...
      while( 1 )
      {
          ...
          {
              ERROR( "No TX time stemp capture for Delay_Req, set timestamp to 0\n" );
              ptpClock->delay_req_send_time.seconds= 0;
              ptpClock->delay_req_send_time.nanoseconds = 0;
          }
          subTime( &diffTime, time, &sendTime );

          if( diffTime.seconds ==0 && diffTime.nanoseconds < 100000000 )        /* 100ms */
          {
              ptpClock->delay_req_send_time = sendTime;
              break;        /* reasonable time stamp */
          }
      }

      DBG( "Get DelayReq sent time %d:%d\n", ptpClock->delay_req_send_time.seconds, ptpClock->delay_req_send_time.nanoseconds );

      /* got good captured timestamp for the outgoing packet */
      if( !( ptpClock->delay_req_send_time.seconds==0 && ptpClock->delay_req_send_time.nanoseconds==0 ) )
      {
          addTime(&ptpClock->delay_req_send_time, &ptpClock->delay_req_send_time, &rtOpts->outboundLatency);
      ...
      {
          ERROR( "Ignore this delay request\n" );
      }
    }
    break;
  ...
  case PTP_SLAVE:
      ...
      && !memcmp(header->sourceUuid, ptpClock->parent_uuid, PTP_UUID_LENGTH) )
    {
      ...
      if( !( ptpClock->delay_req_send_time.seconds ==0 && ptpClock->delay_req_send_time.nanoseconds==0 ) )
      {
        ...
        TimeInternal temp;      /* JW */
        subTime( &temp, &ptpClock->delay_req_receive_time, &ptpClock->delay_req_send_time);
        DBG( "delay is %d:%d\n", temp.seconds, temp.nanoseconds );
