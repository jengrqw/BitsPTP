...
void s1(MsgHeader *header, MsgSync *sync, PtpClock *ptpClock)
{
  ...
  ptpClock->parent_followup_capable  = (Boolean)getFlag(header->flags, PTP_ASSIST);
  ptpClock->parent_external_timing = (Boolean)getFlag(header->flags, PTP_EXT_SYNC);
  ...
  ptpClock->leap_59 =  (Boolean)getFlag(header->flags, PTP_LI_59);
  ptpClock->leap_61 =  (Boolean)getFlag(header->flags, PTP_LI_61);
