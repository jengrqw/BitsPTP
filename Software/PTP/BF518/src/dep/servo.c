...
void bisectTime( TimeInternal* ps_ts, TimeInternal* ps_tsRes );
...
*/
  ...
  bisectTime( &ptpClock->one_way_delay, &ptpClock->one_way_delay );
#if 0    
...
{
    // bisect seconds
    ps_tsRes->seconds   = ps_ts->seconds / 2;
    // bisect nanoseconds and
    ps_tsRes->nanoseconds = ps_ts->nanoseconds / 2;
    // overflow?
    if (( ps_ts->seconds & 0x01 ) == 0x01 )
    {
        if ( ps_ts->nanoseconds < 0 )
        {
            ps_tsRes->nanoseconds -= 500000000;
        }
        else
        {
            ps_tsRes->nanoseconds += 500000000;
