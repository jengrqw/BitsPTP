...
typedef struct {
  ...
  Integer32  	left;
  Boolean 		expire;
  Integer32  	lastTime;			// ZC add to use VDK time
} IntervalTimer;
...
/* This data set is used to qualify messages from */
/* foreign master clocks received by the port.    */
typedef struct
{
...
typedef struct {
  /* Default data set */
  /* This data set defines inherent or assumed properties of the local clock. */
  /* These values are used when the local clock becomes the grandmaster clock */
  /* in a subdomain 	                                                      */
  ...
  /* Current data set */
  /* This data set defines members whose values characterize the current properties */
  /* of the local clock that describe the source and quality of the local time.     */
  /* These values depend on the specific source of time for the local clock and     */
  /* whether it is the grandmaster.                                                 */
  UInteger16  	steps_removed;
  TimeInternal  offset_from_master;
  TimeInternal  one_way_delay;
  
  /* Parent data set */
  /* This data set defines quantities whose values characterize the current         */
  /* properties of the PTP port, the parent or parent port, serving as the source   */
  /* for the time for a PTP slave port via the PTP protocol                         */
  ...
  /* the basis for protocol decisions and providing values for message fields */
